...
def get_file_key(path):
    ...
    if path.startswith(("s3://","arn")):
        try:
            ...
            raise ValueError("Invalid S3 path format — key not found after bucket name.")
        ...
        if path.startswith("https://s3.amazonaws.com/"):
            try:
                ...
                raise ValueError("Invalid S3 URL format — key not found after bucket name.")
        else:
            try:
        ...
        raise ValueError("Path format not recognised. Must be s3://..., arn:..., or https://...")
    
    if len(key.encode("utf-8")) > 1024:
        raise ValueError("S3 object key exceeds the maximum length of 1,024 bytes (UTF-8 encoded)")
